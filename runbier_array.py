...
# Method for sorting - runbier
def sort_and_print_points(points):
    MARGIN_OF_ERROR = 10  # Change as needed

    # Sort the points by their y-values
    points.sort(key=lambda coord: coord[1])

    current_row = [points[0]]
    current_y = points[0][1]

    for coord in points[1:]:
        if abs(coord[1] - current_y) <= MARGIN_OF_ERROR:
...
# Combining list you guys made - runbier
all_detections = yellow_detections + green_detections + purple_detections + white_detections

# Extract only the coordinates from the detections for sorting
all_coords = [detection[0] for detection in all_detections]

# Sort and print the points
sort_and_print_points(all_coords)

# Display the image with dots
display('White Mask', white_mask)
display('Yellow Mask', yellow_mask)
display('Green Mask', green_mask)
display('Purple Mask', purple_mask)
