...
def find_detections(image):
    ...
    for contour in contours:
        ...
        if True:  # TODO: eventually add size thresholding based on distance
            ...
            if 1.5 * cv2.contourArea(contour) > area and cy - b >= 0 and cx - a >= 0 and cy + b < image.shape[
                0] and cx + a < image.shape[1]:
                small_image = image[int(cy - b):int(cy + b), int(cx - a):int(cx + a)]
                mask = np.zeros_like(small_image)
                # writes the donut shape onto the mask
                cv2.ellipse(mask, (int(a), int(b)), (int(outside * a), int(outside * b)), 0, 0.0, 360.0, (1, 1, 1),
                            -1)
                cv2.ellipse(mask, (int(a), int(b)), (int(inside * a), int(inside * b)), 0, 0.0, 360.0, (0, 0, 0),
                            -1)
